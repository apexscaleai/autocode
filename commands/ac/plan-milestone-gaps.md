...
<objective>
Create all phases necessary to close gaps identified by `/ac:audit-milestone`.

Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase.

One command creates all fix phases — no manual `/ac:add-phase` per gap.
</objective>

<execution_context>
<!-- Spawns ac-planner agent which has all planning expertise baked in -->
</execution_context>
...
```
No audit gaps found. Run `/ac:audit-milestone` first.
```
...
**Also available:**
- `/ac:execute-phase {N}` — if plans already exist
...
`/ac:audit-milestone` — re-audit to verify gaps closed
`/ac:complete-milestone {version}` — archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/ac:plan-phase` next
</success_criteria>
