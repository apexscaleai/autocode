...
@~/.claude/autocode/workflows/execute-phase.md
</execution_context>
...
4. **Execute waves**
   ...
   - Spawn `ac-executor` for each plan in wave (parallel Task calls)
...
7. **Verify phase goal**
   ...
   **Otherwise:**
   - Spawn `ac-verifier` subagent with phase directory and goal
   - Verifier checks must_haves against actual codebase (not SUMMARY claims)
   - Creates VERIFICATION.md with detailed report
   - Route by status:
     - `passed` → continue to step 8
     - `human_needed` → present items, get approval or feedback
     - `gaps_found` → present gaps, offer `/ac:plan-phase {X} --gaps`
...
**Also available:**
- /ac:verify-work — manual acceptance testing
...
- /ac:verify-work {Z} — manual testing before planning

───────────────────────────────────────────────────────────────

---

After user runs /ac:plan-phase {Z} --gaps:
1. Planner reads VERIFICATION.md gaps
2. Creates plans 04, 05, etc. to close gaps
3. User runs /ac:execute-phase {Z} again
...
```
Task(prompt="Execute plan at {plan_01_path}\n\nPlan:\n{plan_01_content}\n\nProject state:\n{state_content}", subagent_type="ac-executor", model="{executor_model}")
...
See `@~/.claude/autocode/workflows/execute-phase.md` step `checkpoint_handling` for complete details.
</checkpoint_handling>
