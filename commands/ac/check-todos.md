...
Todos are captured during work sessions with /ac:add-todo.

---

Would you like to:

1. Continue with current phase (/ac:progress)
2. Add a todo now (/ac:add-todo)
```
...
- `/ac:check-todos` → show all
- `/ac:check-todos api` → filter to area:api only
</step>
...
- `/ac:check-todos [area]` to filter by area
- `q` to exit
```
...
- options:
  - "Work on it now" — move to done, start working
  - "Create a phase" — /ac:add-phase with this scope
  - "Brainstorm approach" — think through before deciding
  - "Put it back" — return to list
</step>
...
**Create a phase:**
Display: `/ac:add-phase [description from todo]`
...
- Don't create plans from this command — route to /ac:plan-phase or /ac:add-phase
</anti_patterns>
