...
Provides a better update experience than raw `npx autocode-ac` by showing version diff and changelog entries.
</objective>
...
if [ -f "./.claude/autocode/VERSION" ]; then
  cat "./.claude/autocode/VERSION"
  echo "LOCAL"
elif [ -f ~/.claude/autocode/VERSION ]; then
  cat ~/.claude/autocode/VERSION
  echo "GLOBAL"
else
  echo "UNKNOWN"
...
```bash
npm view autocode-ac version 2>/dev/null
```
...
To update manually: `npx autocode-ac --global`
```
...
- `commands/ac/` will be wiped and replaced
- `autocode/` will be wiped and replaced
- `agents/ac-*` files will be replaced

(Paths are relative to your install location: `~/.claude/` for global, `./.claude/` for local)

Your custom files in other locations are preserved:
- Custom commands not in `commands/ac/` ✓
- Custom agents not prefixed with `ac-` ✓
- Custom hooks ✓
...
**If LOCAL install:**
```bash
npx autocode-ac --local
```

**If GLOBAL install (or unknown):**
```bash
npx autocode-ac --global
```
...
[View full changelog](https://github.com/apexscaleai/autocode/blob/main/CHANGELOG.md)
```
</step>
