...
Output: {phase}-UAT.md tracking all test results. If issues found: diagnosed gaps, verified fix plans ready for /ac:execute-phase
</objective>

<execution_context>
@~/.claude/autocode/workflows/verify-work.md
@~/.claude/autocode/templates/UAT.md
</execution_context>
...
8. If issues found:
   ...
   - Spawn ac-planner in --gaps mode to create fix plans
   - Spawn ac-plan-checker to verify fix plans
   - Iterate planner ↔ checker until plans pass (max 3)
   - Present ready status with `/clear` then `/ac:execute-phase`
</process>
...
- /ac:execute-phase {Z+1} — skip to execution (if already planned)
...
**Also available:**
- /ac:complete-milestone — skip audit, archive directly
...
- /ac:plan-phase {Z} --gaps — regenerate fix plans
...
**Options:**
- /ac:plan-phase {Z} --gaps — retry fix planning with guidance
- /ac:discuss-phase {Z} — gather more context before replanning
...
- [ ] If issues: ac-planner creates fix plans from diagnosed gaps
- [ ] If issues: ac-plan-checker verifies fix plans (max 3 iterations)
- [ ] Ready for `/ac:execute-phase` when complete
</success_criteria>
