...
> **Note:** Planning methodology is in `agents/ac-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/autocode/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/ac:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/autocode/templates/user-setup.md` for full schema and examples
...
See `~/.claude/autocode/workflows/verify-phase.md` for verification logic.
