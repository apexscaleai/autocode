...
# Get templates
PHASE_BRANCH_TEMPLATE=$(cat .planning/config.json 2>/dev/null | grep -o '"phase_branch_template"[[:space:]]*:[[:space:]]*"[^"]*"' | sed 's/.*:.*"\([^"]*\)"/\1/' || echo "ac/phase-{phase}-{slug}")
MILESTONE_BRANCH_TEMPLATE=$(cat .planning/config.json 2>/dev/null | grep -o '"milestone_branch_template"[[:space:]]*:[[:space:]]*"[^"]*"' | sed 's/.*:.*"\([^"]*\)"/\1/' || echo "ac/{milestone}-{slug}")
```
...
**No dependency analysis needed.** Wave numbers are pre-computed during `/ac:plan-phase`.
...
2. **Read files and spawn agents:**
   ...
   **If `PARALLELIZATION=false`:** Spawn agents one at a time, waiting for each to complete before starting the next. This ensures no concurrent file modifications or build operations.
   ...
   @~/.claude/autocode/references/checkpoints.md
   @~/.claude/autocode/references/tdd.md
   </execution_context>
...
1. **Spawn agent for checkpoint plan:**
   ```
   Task(prompt="{subagent-task-prompt}", subagent_type="ac-executor", model="{executor_model}")
   ```
...
| `gaps_found` | Present gap summary, offer `/ac:plan-phase {phase} --gaps` |
...
- `/ac:verify-work {X}` — manual testing before planning
```

User runs `/ac:plan-phase {X} --gaps` which:
1. Reads VERIFICATION.md gaps
2. Creates additional plans (04, 05, etc.) with `gap_closure: true` to close gaps
3. User then runs `/ac:execute-phase {X} --gaps-only`
...
1. Run `/ac:execute-phase {phase}` again
