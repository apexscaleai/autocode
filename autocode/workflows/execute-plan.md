...
@~/.claude/autocode/references/git-integration.md
</required_reading>
...
2. Use Task tool with subagent_type="ac-executor" and model="{executor_model}":
...
2. For each segment in order:
   ...
   B. If routing = Subagent:
      ```
      Spawn Task tool with subagent_type="ac-executor" and model="{executor_model}":
...
See `~/.claude/autocode/references/tdd.md` for TDD plan structure.
</tdd_plan_execution>
...
See ~/.claude/autocode/references/checkpoints.md for complete checkpoint guidance.
</step>
...
Create `.planning/phases/XX-name/{phase}-USER-SETUP.md` using template from `~/.claude/autocode/templates/user-setup.md`.
...
Use ~/.claude/autocode/templates/summary.md for structure.
...
**Also available:**
- `/ac:verify-work {phase}-{plan}` — manual acceptance testing before continuing
...
**Also available:**
- `/ac:verify-work {Z}` — manual acceptance testing before continuing
- `/ac:discuss-phase {Z+1}` — gather context first
...
**Also available:**
- `/ac:verify-work` — manual acceptance testing before completing milestone
- `/ac:add-phase <description>` — add another phase before completing
